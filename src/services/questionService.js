...
//질문이야기 댓글 채택
export async function questionCommentSelect(
    question_id,
    comment_id,
) {
    const token = getAccessToken();
    ...
    try {
        const response = await fetch(
            `/danum-backend/board/question/comment/${question_id}/accept/${comment_id}`,
            {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
...
//질문이야기 댓글 채택 취소
export async function questionCommentDeselect(
    question_id,
    comment_id,
) {
    const token = getAccessToken();
    ...
    try {
        const response = await fetch(
            `/danum-backend/board/question/comment/${question_id}/unaccept/${comment_id}`,
            {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
